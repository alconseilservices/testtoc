...
----------------------------------------------------------------------------------------------
12-Register to receive DESKTOP MOUSE WHEEL event
----------------------------------------------------------------------------------------------

function onDesktopMouseWheel(event)
{
	var direction = event.data; // 1 or -1
	// scrollMyStuff(direction);
}

// register once for receiving Desktop MouseWheel event, as part of your init routine
comm.onDesktopMouseWheel(onDesktopMouseWheel);

----------------------------------------------------------------------------------------------
13- Display Pdf
----------------------------------------------------------------------------------------------

// will display the pdf. Web: on a new window, mobile: using cordova https://www.npmjs.com/package/cordova-plugin-pdf-viewer
comm.displayPdf(url);

Ex: 
function sendPdf(){
	var pdfUrl = computeBaseContainerUrl(); // computeBaseContainerUrl to implement..
	pdfUrl += '/pdf/test.pdf';
	comm.displayPdf(pdfUrl)
}


---------------------------
LOAD CONTENTS URL VARIABLES
---------------------------

Bayam adds a url variable context=bayam when loading external games.
You can use this context to load or not the bayamFrontApi.
Also this url variables is also available:
deviceType=web (web/desktop/ios/android)

From Bayam version 3.0.8, a new url variable is passed if the videos are not autoplayable on the current device and you need to add a play button prior to starting you first video:
startbtn=true // not set when no start button is required
...
{   
	...
	var recurrentEventTypes = [
		BayamFrontCom.EVT_PAUSE,
		BayamFrontCom.EVT_RESUME,
		BayamFrontCom.EVT_VOLUME_CHANGE,
		BayamFrontCom.EVT_DESKTOP_MOUSEWHEEL,
		BayamFrontCom.EVT_GENERIC_GAME_EVENT,
		BayamFrontCom.EVT_ANDROID_BACK_BTN_PRESSED
	]
    ...
    for(var i in pendingEvtCallbacks)
    {
		...
		var type = bayamComEvent.type;
		var isRecurrent = recurrentEventTypes.indexOf(type) > -1;
    	...
    	if(isRecurrent || isAsyncClose || isAsyncReady)
...
BayamFrontCom.EVT_DISPLAY_PDF					= 'evtDisplayPdf';
...
BayamFrontCom.EVT_DESKTOP_MOUSEWHEEL			= 'evtDesktopMouseWheel';
...
function BayamFrontCom(gameId)
{
	...
	var sendEvent = function(bayamComEvent, callbackSuccess, callbackFail)
	{
		...
		if(bfcIsInIFrame)
		{
			try
			{
				window.parent.postMessage(bayamComEvent, '*');
			}
			catch(e)
			{
	...
	/**
	 * Displays a pdf
	 * Web: opens a new window with the pdf
	 * Mobile: uses cordova plugin https://www.npmjs.com/package/cordova-plugin-pdf-viewer
	 * @param {String} url REQUIRED
	 * @param {Function} callbackSuccess OPTIONAL
	 * @param {Function} callbackFail OPTIONAL
	 */
	this.displayPdf = function(url, callbackSuccess, callbackFail)
	{
		if(!url)
			throw new Error('BayamFrontCom::displayPdf : url is required!');
		var evt = new BayamFrontComDisplayPdfEvent(BayamFrontCom.EVT_DISPLAY_PDF, url);
		sendEvent(evt, callbackSuccess, callbackFail);
	}
	 ...
	 * @param {Function} callbackSuccess OPTIONAL
	 * @param {Function} callbackFail OPTIONAL
	 * an instance of BayamFrontComCreationData will be sent with response
	 * callback(event){ var bayamFrontComCreationData = event.data; console.log(bayamFrontComCreationData.url_image); }
	 */
	...
	/**
	 * Register to receive Mousewheel event
	 * @param {Function} callback REQUIRED
	 */
	this.onDesktopMouseWheel = function(callback)
	{
        if(!callback)
                throw new Error('BayamFrontCom::onDesktopMouseWheel : callback method required!');
        var evt = new BayamFrontComEvent(BayamFrontCom.EVT_DESKTOP_MOUSEWHEEL);
        sendEvent(evt, callback);
	}
...
function BayamFrontComDisplayPdfEvent(type, url)
{
	this.url = url;
	BayamFrontComEvent.call(this, type);
}

BayamFrontComDisplayPdfEvent.prototype = Object.create(BayamFrontComEvent.prototype);
...
/* CREATION DATA */
function BayamFrontComCreationData()
{
	this.descriptor = '';
	this.url_image = '';
}
